...
def build_overlap_graph(reads, min_length=3):
    ...
    Returns:
        dict: A dictionary where keys are reads and values are lists of (neighbor_read, overlap_length).
    """
    graph = {}
...
def draw_graph(graph, figure_name):
    ...
    if len(G.nodes) > 250:
        print(f"Graph too large ({len(G.nodes)} nodes) to reasonably draw plot, skipping overlap graph rendering for {figure_name} (would otherwise freeze).")
        return

    # Use explicit subplots for more reliable title rendering
